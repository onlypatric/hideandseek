...
public class Debug implements ICommand {
    ...
    public void execute(Player sender, String[] args) {
        com.patric.mcexp.hideseek.configuration.Map map = Maps.getMap(args[0]);
        if(map == null) {
    ...
    private Inventory createMenu(com.patric.mcexp.hideseek.configuration.Map map, Player sender){
        ...
        debugMenu.setItem(0, createOption(functions, 0, XMaterial.LEATHER_CHESTPLATE.get(), "&6Become a &lHider", 1, player -> {
            ...
            if(Main.getInstance().getGame().getStatus() != com.patric.mcexp.hideseek.game.util.Status.STARTING)
                PlayerLoader.resetPlayer(player, Main.getInstance().getBoard());
        }));
        debugMenu.setItem(1, createOption(functions, 1, XMaterial.GOLDEN_CHESTPLATE.get(), "&cBecome a &lSeeker", 1, player -> {
            if(Config.mapSaveEnabled) {
                if(map.getGameSpawn().getWorld() == null) map.getWorldLoader().loadMap();
            }
        ...
        debugMenu.setItem(2, createOption(functions, 2, XMaterial.IRON_CHESTPLATE.get(), "&8Become a &lSpectator", 1, player -> {
            if(Config.mapSaveEnabled) {
                if(map.getGameSpawn().getWorld() == null) map.getWorldLoader().loadMap();
            }
        ...
        debugMenu.setItem(3, createOption(functions, 3, XMaterial.BARRIER.get(), "&cUnload from Game", 1, player -> {
            Main.getInstance().getBoard().remove(player);
            PlayerLoader.unloadPlayer(player);
            Config.exitPosition.teleport(player);
        }));
        debugMenu.setItem(4, createOption(functions, 4, XMaterial.BARRIER.get(), "&cDie In Game", 2, player -> {
        ...
        if(map.isBlockHuntEnabled()) {
            debugMenu.setItem(7, createOption(functions, 7, XMaterial.GLASS.get(), "&dEnable Disguise", 1, player -> {
                PlayerLoader.openBlockHuntPicker(player, map);
            }));
            debugMenu.setItem(8, createOption(functions, 8, XMaterial.PLAYER_HEAD.get(), "&dDisable Disguise", 1, player -> Main.getInstance().getDisguiser().reveal(player)));
        }
    ...
    public List<String> autoComplete(@NotNull String parameter, @NotNull String typed) {
        if(parameter.equals("map")) {
            return Maps.getAllMaps().stream().map(com.patric.mcexp.hideseek.configuration.Map::getName).collect(Collectors.toList());
        }
        return null;
    }
