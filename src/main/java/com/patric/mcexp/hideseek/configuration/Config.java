...
import com.patric.mcexp.hideseek.game.util.CountdownDisplay;
...
public class Config {
	...
	public static void loadConfig() {
		...
		Optional<XSound> heartbeatOptional = XSound.REGISTRY.getByName(config.getString("seekerPing.sounds.heartbeatNoise"));
		heartbeatSound = heartbeatOptional.orElse(XSound.BLOCK_NOTE_BLOCK_BASEDRUM);
		Optional<XSound> ringingOptional = XSound.REGISTRY.getByName(config.getString("seekerPing.sounds.ringingNoise"));
		...
		for(String id : tempInteracts) {
			...
			if (optional_mat.isPresent()) {
				Material mat = optional_mat.get().get();
				if (mat != null) {
